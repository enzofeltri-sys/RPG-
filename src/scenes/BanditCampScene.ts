...
const ENCOUNTERS: EncounterMarker[] = [
  { id: 'bandits_1', x: 90, y: 100, label: 'Bandits' },
  { id: 'bandits_2', x: 180, y: 70, label: 'Bandits' },
];

interface BanditCampData {
  resume?: boolean;
  x?: number;
  y?: number;
}

// An optional detour off the Champ — a bandit hideout with a wounded guard
// (quest giver) at the safe edge and the bandits themselves further in.
...
export class BanditCampScene extends Phaser.Scene {
  ...
  private talkToGuard(): void {
    ...
    if (progress.state === 'active') {
      this.openDialog(`${quest.title}\n\nProgression : ${progress.progress}/${quest.objective.count} bandits vaincus.`, [
  ...
  private leaveCamp(): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Field', { x: 40, y: 300 });
    });
  }
}
