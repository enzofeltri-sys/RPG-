...
// Vingt-cinquième donjon — une vieille halte de voyageurs le long de la
// route commerciale, où la corruption ressurgit soudainement, comme si elle
// réagissait à l'approche de la silhouette. Première fois depuis le tout
// début de l'Acte 3 que les deux fils de l'enquête (l'identité et le sceau
// qui se brise) se recroisent. Même gabarit "moyen" que les 24 donjons
// précédents, palette corrompue cousine du Bosquet corrompu.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'corrupted_wolf', x: WORLD_WIDTH / 2, y: 320, label: 'Bêtes corrompues' },
  { monsterId: 'corrupted_wolf', x: WORLD_WIDTH / 2, y: 250, label: 'Bêtes corrompues' },
];

const BOSS_MONSTER_ID = 'blight_sentinel';

interface CorruptedWaystationData {
  ...
  // "returning mid-run" from a genuine fresh entry via the road's hidden
  // thicket.
  resume?: boolean;
  x?: number;
  y?: number;
...
export class CorruptedWaystationScene extends Phaser.Scene {
  ...
  constructor() {
    super('CorruptedWaystation');
  }

  init(data: CorruptedWaystationData): void {
    if (!data?.resume) {
  ...
  async create(): Promise<void> {
    ...
    this.cameras.main.setBackgroundColor('#2a2e1c');
    void addStoneFloor(this, WORLD_WIDTH, WORLD_HEIGHT);

    addCrispText(this, this.scale.width / 2, 12, 'La Halte corrompue', {
      fontSize: '10px',
      color: '#9aa0a6',
    })
      .setOrigin(0.5)
    ...
    this.physics.add.overlap(this.player, exitZone, () => this.leaveWaystation());
  ...
  private addThickets(): void {
    // Purely decorative, kept well clear of the center corridor (x=110) that
    // the encounters, gate, and boss zone all sit on.
    const thicket = (x: number, y: number, w: number, h: number) => {
      const rect = this.add.rectangle(x, y, w, h, 0x364022).setStrokeStyle(1, 0x141a0c);
      this.physics.add.existing(rect, true);
      this.physics.add.collider(this.player, rect);
    };
    thicket(20, 360, 24, 60);
    thicket(WORLD_WIDTH - 20, 280, 24, 60);
    thicket(20, 140, 24, 60);
    thicket(WORLD_WIDTH - 20, 360, 24, 60);
  }

  private addGate(): void {
    this.gate = this.add
      .rectangle(WORLD_WIDTH / 2, GATE_Y, WORLD_WIDTH, 16, 0x364022)
      .setStrokeStyle(1, 0x141a0c);
    this.physics.add.existing(this.gate, true);
    this.gateCollider = this.physics.add.collider(this.player, this.gate);
    this.gateLabel = addCrispText(this, WORLD_WIDTH / 2, GATE_Y - 16, 'Ronces corrompues', {
      fontSize: '8px',
      color: '#9aa0a6',
    }).setOrigin(0.5);
  }
  ...
  private addEncounterZone(encounter: EncounterMarker): void {
    const marker = this.add
      .rectangle(encounter.x, encounter.y, 28, 28, 0x3e4a28, 0.8)
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    const marker = this.add.rectangle(x, y, 50, 50, 0x141a0c, 0.85).setStrokeStyle(2, 0xe8d9b5);
    ...
    addCrispText(this, x, y - 36, 'Le puits noirci', {
      fontSize: '9px',
      color: '#e8d9b5',
      align: 'center',
    }).setOrigin(0.5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'CorruptedWaystation', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private leaveWaystation(): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Road', { x: 240, y: 60 });
    });
  }
}
