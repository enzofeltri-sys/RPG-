...
// Vingt-troisième donjon — un vieux cimetière à l'écart de Valombre, que les
// enfants évitent sans qu'on ait besoin de le leur dire. Le premier donjon
// principal de la quête à revenir dans la région de départ depuis la Faille
// du Sceau — le prénom légendaire ramène l'enquête là où tout a commencé.
// Même gabarit "moyen" que les 22 donjons précédents, palette d'herbe grise
// et de pierre tombale plutôt que souterraine.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'corrupted_knight', x: WORLD_WIDTH / 2, y: 320, label: 'Gardiens du cimetière' },
  { monsterId: 'corrupted_knight', x: WORLD_WIDTH / 2, y: 250, label: 'Gardiens du cimetière' },
];

const BOSS_MONSTER_ID = 'grave_warden';

interface ForgottenGraveData {
  ...
  // "returning mid-run" from a genuine fresh entry via the village's hidden
  // cemetery path.
  resume?: boolean;
  x?: number;
  y?: number;
...
export class ForgottenGraveScene extends Phaser.Scene {
  ...
  constructor() {
    super('ForgottenGrave');
  }

  init(data: ForgottenGraveData): void {
    if (!data?.resume) {
  ...
  private addGraves(): void {
    // Purely decorative, kept well clear of the center corridor (x=110) that
    // the encounters, gate, and boss zone all sit on.
    const grave = (x: number, y: number, w: number, h: number) => {
      const rect = this.add.rectangle(x, y, w, h, 0x3a3e34).setStrokeStyle(1, 0x161812);
      this.physics.add.existing(rect, true);
      this.physics.add.collider(this.player, rect);
    };
    grave(20, 360, 24, 60);
    grave(WORLD_WIDTH - 20, 280, 24, 60);
    grave(20, 140, 24, 60);
    grave(WORLD_WIDTH - 20, 360, 24, 60);
  }

  private addGate(): void {
    this.gate = this.add
      .rectangle(WORLD_WIDTH / 2, GATE_Y, WORLD_WIDTH, 16, 0x3a3e34)
      .setStrokeStyle(1, 0x161812);
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    this.add.rectangle(x, y, 50, 50, 0x181a16, 0.85).setStrokeStyle(2, 0xe8d9b5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'ForgottenGrave', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'ForgottenGrave');
    this.chest.setFillStyle(0x3a3428);
  ...
  private leaveGrave(): void {
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Village', { x: 420, y: 550 });
    });
  }
}
