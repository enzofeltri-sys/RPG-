...
const CHEST_ID = 'sanctuarydepths_chest_1';

// The true final dungeon — beneath the original seal chamber itself
// (SealChamberScene), reachable only once Sélène sends the player here for
// the rite (see ShrineScene's FINAL_RITE_STAGES). Same "medium" template as
// every prior dungeon, darker/tighter palette than the others to read as
// the oldest, deepest site in the game.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'corrupted_sentinel', x: WORLD_WIDTH / 2, y: 320, label: 'Ce qui veillait encore' },
  { monsterId: 'corrupted_sentinel', x: WORLD_WIDTH / 2, y: 250, label: 'Ce qui veillait encore' },
];

const BOSS_MONSTER_ID = 'demon_king_echo';

interface SanctuaryDepthsData {
  ...
  // "returning mid-run" from a genuine fresh descent through the rite
  // fissure in ShrineScene.
  resume?: boolean;
  x?: number;
  y?: number;
}

export class SanctuaryDepthsScene extends Phaser.Scene {
  ...
  constructor() {
    super('SanctuaryDepths');
  }

  init(data: SanctuaryDepthsData): void {
    if (!data?.resume) {
  ...
  async create(): Promise<void> {
    ...
    this.cameras.main.setBackgroundColor('#15121c');

    addCrispText(this, this.scale.width / 2, 12, 'Sous le Sceau originel', {
      fontSize: '10px',
      color: '#9aa0a6',
    })
      .setOrigin(0.5)
  ...
  private addPillars(): void {
    // Purely decorative, kept well clear of the center corridor (x=110)
    // that the encounters, gate, and boss zone all sit on.
    const pillar = (x: number, y: number, w: number, h: number) => {
      const rect = this.add.rectangle(x, y, w, h, 0x2a2436).setStrokeStyle(1, 0x0e0c14);
  ...
  private addGate(): void {
    this.gate = this.add
      .rectangle(WORLD_WIDTH / 2, GATE_Y, WORLD_WIDTH, 16, 0x2a2436)
      .setStrokeStyle(1, 0x0e0c14);
    this.physics.add.existing(this.gate, true);
    this.gateCollider = this.physics.add.collider(this.player, this.gate);
    this.gateLabel = addCrispText(this, WORLD_WIDTH / 2, GATE_Y - 16, 'Barrière du rite', {
      fontSize: '8px',
      color: '#9aa0a6',
    }).setOrigin(0.5);
  }
  ...
  private addBossZone(): void {
    ...
    addCrispText(this, x, y - 36, 'Le cœur du Sceau', {
      fontSize: '9px',
      color: '#e8d9b5',
      align: 'center',
    }).setOrigin(0.5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'SanctuaryDepths', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'SanctuaryDepths');
    this.chest.setFillStyle(0x3a3428);
    void attachSpriteOverlay(this, this.chest, 'decor-treasure_chest_open', `${import.meta.env.BASE_URL}sprites/decor/treasure_chest_open.png`, 16);
  ...
  private leaveDepths(): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Shrine', { x: 180, y: 300 });
    });
  }
}
