...
export class SealedSanctuaryScene extends Phaser.Scene {
  ...
  async create(): Promise<void> {
    ...
    // there — the Limaneux found it during a later sweep of the vault, well
    // clear of the boss zone so it never interferes with that fight.
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    const marker = this.add.rectangle(x, y, 50, 50, 0x1f2a3a, 0.85).setStrokeStyle(2, 0xe8d9b5);
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'SealedSanctuary');
    this.chest.setFillStyle(0x3a3428);
    void attachSpriteOverlay(this, this.chest, 'decor-treasure_chest_open', `${import.meta.env.BASE_URL}sprites/decor/treasure_chest_open.png`, 16);
  ...
  private enterShardSeekersCamp(): void {
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('ShardSeekersCamp', { x: 110, y: 380 });
    });
  }
