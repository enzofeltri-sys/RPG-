...
const CHEST_ID = 'shardseekerscamp_chest_1';

// Same gabarit as the rest of the "moyen" dungeons (gate behind two fixed
// encounters, boss zone beyond it, one chest) — the main quest's current
// hardest dungeon, tucked behind the Sealed Sanctuary's own hidden passage
// since the Chercheurs d'éclats needed somewhere nobody would think to look
// twice.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'seeker_scout', x: WORLD_WIDTH / 2, y: 320, label: 'Éclaireurs des Chercheurs' },
  { monsterId: 'seeker_scout', x: WORLD_WIDTH / 2, y: 250, label: 'Éclaireurs des Chercheurs' },
];

const BOSS_MONSTER_ID = 'seeker_archivist';

interface ShardSeekersCampData {
  ...
  // mid-run" from a genuine fresh entry via the Sealed Sanctuary's hidden
  // passage.
  resume?: boolean;
  x?: number;
  y?: number;
...
export class ShardSeekersCampScene extends Phaser.Scene {
  ...
  constructor() {
    super('ShardSeekersCamp');
  }

  init(data: ShardSeekersCampData): void {
    if (!data?.resume) {
  ...
  private addCrates(): void {
    ...
    const crate = (x: number, y: number, w: number, h: number) => {
      const rect = this.add.rectangle(x, y, w, h, 0x3a2a40).setStrokeStyle(1, 0x1c1420);
      this.physics.add.existing(rect, true);
      this.physics.add.collider(this.player, rect);
    };
    crate(20, 360, 24, 60);
    crate(WORLD_WIDTH - 20, 280, 24, 60);
    crate(20, 140, 24, 60);
  }

  private addGate(): void {
    this.gate = this.add
      .rectangle(WORLD_WIDTH / 2, GATE_Y, WORLD_WIDTH, 16, 0x3a2a4a)
      .setStrokeStyle(1, 0x1c1122);
    this.physics.add.existing(this.gate, true);
    this.gateCollider = this.physics.add.collider(this.player, this.gate);
    this.gateLabel = addCrispText(this, WORLD_WIDTH / 2, GATE_Y - 16, 'Étagères barricadées', {
      fontSize: '8px',
      color: '#9aa0a6',
    }).setOrigin(0.5);
  }
  ...
  private addEncounterZone(encounter: EncounterMarker): void {
    const marker = this.add
      .rectangle(encounter.x, encounter.y, 28, 28, 0x4a3a5a, 0.8)
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    const marker = this.add.rectangle(x, y, 50, 50, 0x2a1f3a, 0.85).setStrokeStyle(2, 0xe8d9b5);
    ...
    addCrispText(this, x, y - 36, "Salle de l'archiviste", {
      fontSize: '9px',
      color: '#e8d9b5',
      align: 'center',
    }).setOrigin(0.5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'ShardSeekersCamp', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'ShardSeekersCamp');
    this.chest.setFillStyle(0x3a3428);
    void attachSpriteOverlay(this, this.chest, 'decor-treasure_chest_open', `${import.meta.env.BASE_URL}sprites/decor/treasure_chest_open.png`, 16);
  ...
  private leaveCamp(): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('SealedSanctuary', { x: 190, y: 40 });
    });
  }
}
