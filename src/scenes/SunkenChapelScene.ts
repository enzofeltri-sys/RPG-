...
const CHEST_ID = 'sunkenchapel_chest_1';

// Vingt-sixième donjon — une chapelle engloutie sous les vieux quais du
// Faubourg, bâtie à la même époque que le Sceau originel : le premier site
// confirmé, et non plus seulement théorisé, d'un rite pensé pour plusieurs
// mains à plusieurs endroits. Même gabarit "moyen" que les 25 donjons
// précédents, palette d'eau trouble et de pierre noyée.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'marsh_serpent', x: WORLD_WIDTH / 2, y: 320, label: 'Serpents des eaux noires' },
  { monsterId: 'marsh_serpent', x: WORLD_WIDTH / 2, y: 250, label: 'Serpents des eaux noires' },
];

const BOSS_MONSTER_ID = 'sunken_warden';

interface SunkenChapelData {
  ...
  // "returning mid-run" from a genuine fresh entry via the Faubourg's
  // hidden dock passage.
  resume?: boolean;
  x?: number;
  y?: number;
...
export class SunkenChapelScene extends Phaser.Scene {
  ...
  init(data: SunkenChapelData): void {
    if (!data?.resume) {
  ...
  async create(): Promise<void> {
    ...
    addCrispText(this, this.scale.width / 2, 12, 'La Chapelle engloutie', {
      fontSize: '10px',
      color: '#9aa0a6',
    })
      .setOrigin(0.5)
    ...
    this.physics.add.overlap(this.player, exitZone, () => this.leaveChapel());
  ...
  private addEncounterZone(encounter: EncounterMarker): void {
    const marker = this.add
      .rectangle(encounter.x, encounter.y, 28, 28, 0x2c3a40, 0.8)
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    const marker = this.add.rectangle(x, y, 50, 50, 0x0e1418, 0.85).setStrokeStyle(2, 0xe8d9b5);
    ...
    addCrispText(this, x, y - 36, "Le second autel", {
      fontSize: '9px',
      color: '#e8d9b5',
      align: 'center',
    }).setOrigin(0.5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'SunkenChapel', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'SunkenChapel');
    this.chest.setFillStyle(0x3a3428);
    void attachSpriteOverlay(this, this.chest, 'decor-treasure_chest_open', `${import.meta.env.BASE_URL}sprites/decor/treasure_chest_open.png`, 16);
  ...
  private leaveChapel(): void {
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Faubourg', { x: 30, y: 200, resume: true });
    });
  }
}
