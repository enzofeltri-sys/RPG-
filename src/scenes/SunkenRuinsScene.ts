...
const CHEST_ID = 'sunkenruins_chest_1';
...
// MarshLair/Entrepôt : une porte à deux rencontres, un boss au butin garanti,
// pas de récompense signature — l'écho de vasenoire_ruins dans le donjon lui-
// même plutôt qu'une simple rencontre ambiante.
...
const ENCOUNTERS: EncounterMarker[] = [
  { monsterId: 'bog_wraith', x: WORLD_WIDTH / 2, y: 320, label: 'Spectres' },
  { monsterId: 'bog_wraith', x: WORLD_WIDTH / 2, y: 250, label: 'Spectres' },
];

const BOSS_MONSTER_ID = 'ruins_delver';

interface SunkenRuinsData {
  ...
  // mid-run" from a genuine fresh entry via SunkenRoad's ruins.
  resume?: boolean;
  x?: number;
  y?: number;
}

export class SunkenRuinsScene extends Phaser.Scene {
  ...
  init(data: SunkenRuinsData): void {
    if (!data?.resume) {
  ...
  async create(): Promise<void> {
    ...
    addCrispText(this, this.scale.width / 2, 12, 'Ruines englouties', {
      fontSize: '10px',
      color: '#9aa0a6',
    })
      .setOrigin(0.5)
    ...
    this.physics.add.overlap(this.player, exitZone, () => this.leaveSunkenRuins());
    ...
    // What looked like the bottom of the ruins was only a ceiling — a
    // stairwell down, easy to miss unless you already know it's there.
    const hiddenZone = this.add.zone(30, 15, 40, 20);
    this.physics.add.existing(hiddenZone, true);
    this.physics.add.overlap(this.player, hiddenZone, () => this.enterBrotherhoodTomb());
    addCrispText(this, 30, 28, 'Escalier ↑', { fontSize: '8px', color: '#9aa0a6' }).setOrigin(0.5);
  ...
  private addEncounterZone(encounter: EncounterMarker): void {
    const marker = this.add
      .rectangle(encounter.x, encounter.y, 28, 28, 0x2e4a44, 0.8)
  ...
  private addBossZone(): void {
    ...
    const y = 70;
    const marker = this.add.rectangle(x, y, 50, 50, 0x1f3a3f, 0.85).setStrokeStyle(2, 0xe8d9b5);
    ...
    addCrispText(this, x, y - 36, 'Cœur des ruines', {
      fontSize: '9px',
      color: '#e8d9b5',
      align: 'center',
    }).setOrigin(0.5);
  ...
  private startCombat(monsterId: string): void {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Combat', { returnScene: 'SunkenRuins', monsterId, x: this.player.x, y: this.player.y });
    });
  }
  ...
  private async handleChestTap(): Promise<void> {
    ...
    const loot = openChest(this.character, CHEST_ID, 'SunkenRuins');
    this.chest.setFillStyle(0x3a3428);
    void attachSpriteOverlay(this, this.chest, 'decor-treasure_chest_open', `${import.meta.env.BASE_URL}sprites/decor/treasure_chest_open.png`, 16);
  ...
  private enterBrotherhoodTomb(): void {
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('BrotherhoodTomb', { x: 110, y: 380 });
    });
  }

  private leaveSunkenRuins(): void {
    if (this.isTransitioning) return;
    this.isTransitioning = true;
    this.cameras.main.fadeOut(300, 0, 0, 0);
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('SunkenRoad', { x: 300, y: 320 });
    });
  }
}
